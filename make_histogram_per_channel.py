...
if __name__ == '__main__':
    ...
    for iz in range(NUM_CHANNELS):
        ...
        for ir in range(1000):
            the_dictionary[str(ir)+"-"+str(ir+1)+" away"] = 0
            buckets.append([avg+ir*std_dev, avg+(ir+1)*std_dev])

        for ij in a:
            diff = abs(float(avg-ij)/float(-1*std_dev))
        ...
        with open("histogram_data"+str(iz)+".txt", 'w') as f:
        ...
        plt.title('Standard Deviation vs Count for ION CHANNEL '+str(iz))
        plt.savefig("STD_DEV_VS_COUNT_ION_CHANNEL_"+str(iz)+".png")
        y_value = []
