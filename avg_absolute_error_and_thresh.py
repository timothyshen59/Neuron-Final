...
if __name__ == '__main__':
    ...
    print(total_sum)
    while(fifty_percent != True and threshold < 20):
        the_violating_regions = []
        non_violating_regions = []
        the_sum = 0
        ...
        if(float(the_sum)/float(total_sum) >= 0.45 and (float(the_sum)/float(total_sum) <= 0.55)):
            fifty_percent = True
            break
        else:
            threshold += 0.01
    ...
    plt.savefig('Absolute_AVG_ION_CHANNEL_'+the_lister[ION_CHANNEL]+'_the_data2_'+'.png')
    print("DONE")
